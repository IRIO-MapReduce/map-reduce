...
/**
 * A queue of work items, held by a worker server to serve requests to client
 * binaries.
 */
class WorkQueue {
public:
    ...
    /**
     * Returns any work item that can be processed by worker with a given
     * execpath. Assumes that there is at least one item in the queue.
     */
...
public:
    ...
    {
        /**
         * TODO: Should definitely never happen if I underestand c++ reference
         * correctly.
         */
    ...
    /**
     * Register a job request. Should be called num_jobs times, and all jobs ids
     * should be distinct and in range [0, num_jobs).
     */
     ...
     * Should be called after wait_for_completion, to potentially repeat
     * unfinished jobs. There is a possibility that result contains jobs that
     * finished in the meantime, but all jobs that are not completed are
