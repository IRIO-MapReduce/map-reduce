    ...
    void get_job_manager_address()
    {
        while (true) {
            ...
            if (job_manager_ip.has_value()) {
                job_manager_address
                    = get_address(job_manager_ip.value(), JOB_MANAGER_PORT);
                log_message("[WORKER] Job manager address acquired: "
                    + job_manager_address);
                return;
            }

            log_message(
                "[WORKER] Job manager not found. Retrying in 5 seconds...",
...
int main()
{
    HealthCheckServiceImpl health_service(
        get_address(LISTENING_ADDRESS, HEALTH_CHECK_PORT));
    std::thread health_service_thread(
        &HealthCheckServiceImpl::start, &health_service);
